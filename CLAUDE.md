...
Repo: claude-desktop-mcp-bridge (public), current releases include v0.9.0.
...
1. **compliance.scan_repo** — Runs gitleaks + npm audit + checkov, normalizes findings, produces manifest. Supports `framework: 'soc2' | 'hipaa'` (default soc2). SOC2: "Scanner Reach" on 20 controls. HIPAA: dual metrics — technical reach on 164.312 (12 controls) + 164.308 admin placeholders (7 controls, human evidence required)
...
8. **compliance.create_demo_fixture** — Generates a safe demo repo with intentional findings for all scanners and is gitignored. Supports `preset: 'soc2-demo' | 'hipaa-demo'` (default soc2-demo)
...
- Framework badge (SOC2/HIPAA) displayed in header based on loaded run
...
- `getLatestRunId` returns unsanitized directory names (mitigated by assertCompliancePath downstream; resource handler now validates runId from URI query params with safeRunId regex since v0.9.0)
...
3. ~~**HIPAA Security Rule mapping**~~ — SHIPPED in v0.9.0 (`hipaa-map.ts` + `coverage-shared.ts`, 12 technical controls + 7 admin placeholders, dual reach metrics, 91-assertion E2E suite)
4. **PHI/PII detector** — new scanner (Presidio or similar) alongside gitleaks/npm-audit/checkov. Next target.
...
- `src/compliance-bridge/soc2-map.ts` — 20-control SOC2 mapping (delegates to coverage-shared.ts)
- `src/compliance-bridge/hipaa-map.ts` — 19 HIPAA controls (12 technical + 7 admin placeholders)
- `src/compliance-bridge/coverage-shared.ts` — Shared coverage computation for SOC2 and HIPAA
