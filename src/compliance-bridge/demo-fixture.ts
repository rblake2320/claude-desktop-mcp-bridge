...
const FIXTURE_README_HIPAA = `# HIPAA Security Rule Demo Repo
...
This demo repo was generated by \`compliance.create_demo_fixture\` with the \`hipaa-demo\` preset
to test the HIPAA Security Rule mapping workflow:

1. **Scan**: \`compliance.scan_repo({ repoPath: "<this-dir>", framework: "hipaa" })\`
2. **Audit Packet**: \`compliance.generate_audit_packet({ repoPath: "<this-dir>" })\`
3. **Remediation**: \`compliance.plan_remediation({ repoPath: "<this-dir>" })\`
4. **Dashboard**: \`compliance.open_dashboard({ repoPath: "<this-dir>" })\`

## HIPAA Coverage

This fixture produces findings mapped to **164.312 Technical Safeguards**:
...
**164.308 Administrative Safeguards** are listed as placeholders requiring human evidence.

All secrets are fake test values. All IaC configs are marked DO-NOT-DEPLOY.
`;

function getHipaaDemoFiles(): FileEntry[] {
  return [
    { relativePath: 'README.md', content: FIXTURE_README_HIPAA },
...
export function createDemoFixture(input: CreateDemoFixtureInput): CreateDemoFixtureResponse {
  ...
  const files = input.preset === 'hipaa-demo' ? getHipaaDemoFiles() : getSoc2DemoFiles();
